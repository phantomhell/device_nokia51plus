...
#define S5K4H7_LSC_SIZE		1873
#define S5K4H7_INFO_SIZE		41
...
static int cam_cal_read(u32 addr, u32 size, u8 *data)
{
    ...
    if (addr == 0x00000005)//Group1 EEPROM page21 page 23~52
    {
        ...
        memcpy(data+35, eeprom_4h7_data_LSC, S5K4H7_LSC_SIZE);
        printk("S5K4H7 cam_cal_read Read AWB Golden EEPROM\n");
        printk("S5K4H7 cam_cal_read Read = 0x%x\n",data[0]);
        printk("S5K4H7 cam_cal_read Read = 0x%x\n",data[1]);
        printk("S5K4H7 cam_cal_read Read = 0x%x\n",data[2]);
    ...
    if (addr == 0x00000006)//identify module vendor
    {
        memcpy(data, eeprom_4h7_data, size);
        printk("S5K4H7 cam_cal_read PartNumber = 0x%x\n",data[4]);
        if (data[4] == 0x02)
           printk("S5K4H7 cam_cal_read module_id 2(Truly)\n");
        else if (data[4] == 0x03)
           printk("S5K4H7 cam_cal_read module_id 3(ShineTek)\n");
        else
        	printk("S5K4H7 cam_cal_read module_id Unknown\n");
        return 4;
    }
	return 0;
}
...
void CAM_CAL_GetOtpData_4h7(void)
{
	...
	get_4h7_page_data();
	for(i =0; i < 5; i++)
	snprintf(fih_s5k4h7yx_sernum,sizeof(fih_s5k4h7yx_sernum),"%s%02X", fih_s5k4h7yx_sernum, eeprom_4h7_data[36 + i]);
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    eeprom_4h7_data = kmalloc(S5K4H7_INFO_SIZE, GFP_KERNEL);
    if (NULL == eeprom_4h7_data) {
        CAM_CALDB(" allocate eeprom mem failed\n");
        return -ENOMEM;
    }
    eeprom_4h7_data_LSC = kmalloc(S5K4H7_LSC_SIZE, GFP_KERNEL);
